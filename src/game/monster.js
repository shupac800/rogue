 ...
 * @typedef {{ x:number, y:number, hp:number, maxHp:number, attack:number, defense:number, xp:number, name:string, char:string, aggression:number, provoked:boolean }} Monster
 */
...
export function createMonster(template, x, y) {
  ...
  return { x, y, hp, maxHp: hp, attack, defense, xp, name: name.toLowerCase(), char, aggression, provoked: false };
}
